...
async def test_retry_with_backoff_exhausted(chat_service):
    ...
    with pytest.raises(httpx.TimeoutException):
        await chat_service._retry_with_backoff(
            mock_operation,
            "test_operation"
        )
...
async def test_connection_error_handling(chat_service):
    ...
    with patch.object(chat_service, '_make_api_request', 
...
async def test_loading_indicator_shown(chat_service):
    ...
    with patch.object(chat_service, '_make_api_request', side_effect=mock_slow_operation):
        messages = []
        async for chunk in chat_service.stream_chat_response(
            "test query",
            []

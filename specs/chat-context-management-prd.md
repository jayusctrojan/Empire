...
| Metric | Target |
|--------|--------|
| Context preservation accuracy | >95% of key decisions/code retained (Sonnet) |
| Auto-compaction latency | 15-30s typical, 60s max (Sonnet); 5-10s (Haiku --fast) |
...
- [ ] Manual "/compact" command available with optional "--fast" flag
- [ ] Summarization completes in 15-30 seconds typical, 60 seconds max (Sonnet); 5-10 seconds with --fast (Haiku)
- [ ] Compaction runs in background with progress indicator, allowing user to continue reading/composing

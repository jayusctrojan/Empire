-- Migration: Create RLS Context Function
-- Task 151: Implement RLS Database Context
-- Created: 2025-01-14
--
-- This migration creates the set_rls_context function that sets
-- PostgreSQL session variables for Row-Level Security enforcement.
-- The middleware calls this function on each authenticated request.

-- Create the function to set RLS context variables
CREATE OR REPLACE FUNCTION set_rls_context(
    p_user_id TEXT,
    p_role TEXT,
    p_request_id TEXT
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER  -- Run with function owner's privileges
AS $$
DECLARE
    result JSONB;
BEGIN
    -- Validate inputs
    IF p_user_id IS NULL OR p_user_id = '' THEN
        RAISE EXCEPTION 'user_id cannot be null or empty';
    END IF;

    IF p_role IS NULL OR p_role = '' THEN
        p_role := 'guest';
    END IF;

    IF p_request_id IS NULL OR p_request_id = '' THEN
        p_request_id := gen_random_uuid()::TEXT;
    END IF;

    -- Set session variables for RLS policies
    -- These are local to the current transaction/session
    PERFORM set_config('app.current_user_id', p_user_id, true);  -- true = local to transaction
    PERFORM set_config('app.user_role', p_role, true);
    PERFORM set_config('app.request_id', p_request_id, true);
    PERFORM set_config('app.rls_context_set', 'true', true);

    -- Build result JSON
    result := jsonb_build_object(
        'success', true,
        'user_id', p_user_id,
        'role', p_role,
        'request_id', p_request_id,
        'set_at', NOW()
    );

    RETURN result;
END;
$$;

-- Create function to get current RLS context
CREATE OR REPLACE FUNCTION get_rls_context()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    result JSONB;
BEGIN
    result := jsonb_build_object(
        'user_id', current_setting('app.current_user_id', true),
        'role', current_setting('app.user_role', true),
        'request_id', current_setting('app.request_id', true),
        'context_set', current_setting('app.rls_context_set', true) = 'true'
    );

    RETURN result;
END;
$$;

-- Create function to clear RLS context (for testing/cleanup)
CREATE OR REPLACE FUNCTION clear_rls_context()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    -- Reset session variables
    PERFORM set_config('app.current_user_id', '', true);
    PERFORM set_config('app.user_role', 'guest', true);
    PERFORM set_config('app.request_id', '', true);
    PERFORM set_config('app.rls_context_set', 'false', true);

    RETURN jsonb_build_object('success', true, 'cleared_at', NOW());
END;
$$;

-- Create helper function for RLS policies to get current user ID
CREATE OR REPLACE FUNCTION current_user_id()
RETURNS TEXT
LANGUAGE plpgsql
STABLE  -- Same result within single query
AS $$
BEGIN
    RETURN NULLIF(current_setting('app.current_user_id', true), '');
END;
$$;

-- Create helper function for RLS policies to check if user has role
CREATE OR REPLACE FUNCTION has_role(required_role TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
    user_role TEXT;
    role_hierarchy JSONB := '{"admin": 4, "editor": 3, "viewer": 2, "guest": 1}'::JSONB;
BEGIN
    user_role := COALESCE(current_setting('app.user_role', true), 'guest');

    -- Check if user's role is >= required role in hierarchy
    RETURN (role_hierarchy->>user_role)::INT >= (role_hierarchy->>required_role)::INT;
END;
$$;

-- Grant execute permissions to authenticated users
GRANT EXECUTE ON FUNCTION set_rls_context(TEXT, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_rls_context() TO authenticated;
GRANT EXECUTE ON FUNCTION clear_rls_context() TO authenticated;
GRANT EXECUTE ON FUNCTION current_user_id() TO authenticated;
GRANT EXECUTE ON FUNCTION has_role(TEXT) TO authenticated;

-- Also grant to service role for admin operations
GRANT EXECUTE ON FUNCTION set_rls_context(TEXT, TEXT, TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION get_rls_context() TO service_role;
GRANT EXECUTE ON FUNCTION clear_rls_context() TO service_role;
GRANT EXECUTE ON FUNCTION current_user_id() TO service_role;
GRANT EXECUTE ON FUNCTION has_role(TEXT) TO service_role;

-- Add comment for documentation
COMMENT ON FUNCTION set_rls_context IS
'Sets PostgreSQL session variables for Row-Level Security enforcement.
Called by the RLS context middleware on each authenticated request.
Task 151: Implement RLS Database Context.';

COMMENT ON FUNCTION get_rls_context IS
'Returns the current RLS context as JSON. Useful for debugging.';

COMMENT ON FUNCTION clear_rls_context IS
'Clears the current RLS context. Used for testing and cleanup.';

COMMENT ON FUNCTION current_user_id IS
'Helper function for RLS policies to get the current user ID from session context.';

COMMENT ON FUNCTION has_role IS
'Helper function for RLS policies to check if user has a specific role or higher.';

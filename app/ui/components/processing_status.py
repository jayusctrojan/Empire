...
def create_document_upload_status_component() -> gr.Blocks:
    ...
    with gr.Blocks(css=PROCESSING_STATUS_CSS) as upload_status:
        _document_id = gr.State(value=None)  # noqa: F841
        ...
        with gr.Row():
            _filename_display = gr.Textbox(  # noqa: F841
                label="File",
                value="No file selected",
                interactive=False
            )

            _elapsed_time = gr.Textbox(  # noqa: F841
                label="Elapsed Time",
                value="--:--",
                interactive=False
            )
...
def create_query_status_component() -> gr.Blocks:
    ...
    with gr.Blocks(css=PROCESSING_STATUS_CSS) as query_status:
        _query_id = gr.State(value=None)  # noqa: F841
